(function() {
  ...
  // ── Google Analytics 4 (gtag.js) ─ measurement ID G-CCC51JPRG0 ──
  (function(){
    if (document.getElementById('ga4-gtag')) return;
    var ga = document.createElement('script');
    ga.id = 'ga4-gtag';
    ga.async = true;
    ga.src = 'https://www.googletagmanager.com/gtag/js?id=G-CCC51JPRG0';
    document.head.appendChild(ga);
    window.dataLayer = window.dataLayer || [];
    function gtag(){ dataLayer.push(arguments); }
    window.gtag = gtag;
    gtag('js', new Date());
    gtag('config', 'G-CCC51JPRG0');
  })();

  // ── Microsoft Clarity ─ project yanoxjx16u ──
  //    Masking mode is set to STRICT in the Clarity dashboard (Settings > Masking).
  //    The Introduction form collects marital status, financial independence,
  //    location and photographs. Nothing a visitor types on this site is recorded.
  (function(){
    ...
    cl.id = 'ms-clarity';
    cl.async = true;
    cl.src = 'https://www.clarity.ms/tag/yanoxjx16u';
    document.head.appendChild(cl);
  })();

  // ── GA4 custom events ─ the six specced in TLT2-STRATEGIC-DIRECTION.md §6 ──
  //    All six live here so there is one place to change them. They attach by
  //    delegation and never touch the Introduction form's own submit code.
  (function(){
    ...
    // founding_viewed — someone reached the Founding Member page
    if (path === '/founding-member') ev('founding_viewed');

    // checkout_started — a paid checkout link was clicked, on any page
    document.addEventListener('click', function(e){
      var a = closestFrom(e.target, 'a[href*="krpy.me"]');
      if (!a) return;
      ev('checkout_started', {
        checkout_type: /WNFVj/.test(a.getAttribute('href') || '') ? 'boston_seat_50' : 'membership_444',
        page_path: path
      });
    }, true);

    if (path !== '/member-introduction-page') return;

    // intro_started — the first real interaction with the form, fired once
    var started = false;
    function markStarted(){ if (started) return; started = true; ev('intro_started'); }
    ['input','change'].forEach(function(t){
      document.addEventListener(t, function(e){
        if (closestFrom(e.target, '#form-state')) markStarted();
      }, true);
    });

    // intro_field_photo / intro_field_video — a file was actually chosen
    document.addEventListener('change', function(e){
      var el = e.target;
      if (!el || el.type !== 'file' || !el.files || !el.files.length) return;
      if (el.id === 'photo-input' || el.id === 'photo-input-change') { markStarted(); ev('intro_field_photo'); }
      if (el.id === 'video-input' || el.id === 'video-input-change') { markStarted(); ev('intro_field_video'); }
    }, true);

    // intro_completed — the success screen became visible. Watched rather than
    // hooked, so a change to showSuccess() cannot silently break the event.
    function watchSuccess(){
      var s = document.getElementById('success-screen');
      if (!s || !window.MutationObserver) return;
      var fired = false;
      new MutationObserver(function(){
        if (fired) return;
        if (s.style.display && s.style.display !== 'none') { fired = true; ev('intro_completed'); }
      }).observe(s, { attributes: true, attributeFilter: ['style'] });
    }
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', watchSuccess);
    } else {
      watchSuccess();
    }
  })();
  ...
  var fcFix = document.createElement('style');
  fcFix.textContent = '@media(max-width:900px){.fc-bar-inner{flex-wrap:wrap;height:auto!important;padding:6px 12px;gap:6px 10px;}.fc-text{white-space:normal;text-align:center;}}';
  document.head.appendChild(fcFix);
  ...
  mobileOverlay.innerHTML = [
    ...
    '<a href="https://www.trueluvtake2.com/member-introduction-page" class="btn-primary-mob">Begin Your Free Introduction</a>',

    '<div class="mobile-divider"></div>',
    '<a href="https://www.trueluvtake2.com/how-it-works" class="mob-primary">How It Works</a>',
    '<a href="https://www.trueluvtake2.com/love-team" class="mob-primary">The Love Team</a>',
  ...
  if (!nav || !hamburger || !mobile) {
    ...
    // Live counter retired 22 Aug 2026 — the bar states the offer, not the progress.
  // Bring it back when there are real Boston members to count.
  }
